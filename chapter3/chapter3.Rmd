...
# Question 2
  # a) the unit of observation is the class-by-school
  # b) 2019 observations
  dim(grades)
  # c) You see that each variable is labeled in the `View` table
  View(grades)
  # d) avgmath = "average composite math score in class",
  #    avgverb = "average composite verbal score in class"
  # e) 
  library(skimr)
  grades %>% 
...
# Question 3 (don't visualize)
  # grades_avg_cs %>% 
  #   select(classize, avgmath_cs, avgverb_cs) %>% 
  #   cor
  #            classize avgmath_cs avgverb_cs
  # classize   1.0000000  0.5244569  0.3420341
  # avgmath_cs 0.5244569  1.0000000  0.6428880
  # avgverb_cs 0.3420341  0.6428880  1.0000000
```
...
```{R, eval=FALSE,echo=FALSE}
# Question 1
  x <- grades_avg_cs$classize
  y <- grades_avg_cs$avgmath_cs
...
# Question 4
  hat_y_x_0 <- beta_0 + beta_1 * 0
  hat_y_x_0
  # Does it make sense? What's the average grade in a class with no students?
  
# Question 5
  hat_y_x_30 <- beta_0 + beta_1 * 30
  hat_y_x_30
...
* All are described [in the book](https://michelefioretti.github.io/ScPoEconometrics/linreg.html#OLS). Optional `r emo::ji("nerd_face")`.
...
# Question 3
  cor(x,y) #--> cor(x,y) = r = sqrt(R^2)
  # Check that this is true!
  round(sqrt(summary(math_reg)$r.squared), 4) == round(cor(x,y), 4)

# Question 4
  # as by the question using the `broom` package
  install.packages("broom") # install the package
  library(broom)            # load the package
  
  math_reg_broomed <- broom::augment(math_reg)
  
  math_reg_broomed # math_reg is a tibble containing lots of interesting info
  
  var(math_reg_broomed$.fitted) / var(math_reg_broomed$y) # =  0.275055
  1 - var(math_reg_broomed$.resid) / var(math_reg_broomed$y) # =  0.275055
  
  # alternative 1
    # computing them by hand
    beta_1 <- cov(x,y)/var(x)
    beta_0 <- mean(y) - beta_1 * mean(x)
    fitted <- beta_0 + beta_1 * x
    residual <- y - fitted
    var(fitted)/var(y)  # = 0.275055
    1 - var(residual)/var(y) # = 0.275055
  # alternative 2
    # `lm` objects are `list`s, which contains other objects. 
    # we can find both `fitted.values`, `residuals` and the `y` (as well as `x`) inside an `lm` object
    var(math_reg$fitted.values) / var(math_reg$model$y)  # = 0.275055
    1 - var(math_reg$residuals) / var(math_reg$model$y)  # = 0.275055
  
  # alternative 3
    # using the `fitted(.)` and `residuals(.)` functions
    ?fitted
    ?residuals
    var(fitted(math_reg)) / var(math_reg$model$y) # = 0.275055
    1 - var(residuals(math_reg)) / var(math_reg$model$y) # = 0.275055
  
 # Question 5  
    y_verb <- grades_avg_cs$avgverb_cs 
    
    verb_reg <- lm(y_verb ~ x)
    # compare math_reg with verb_reg: 
        # a) which regression courve is flatter? 
        # b) what's the average verbal score in a class with no students?
    summary(math_reg)
    summary(verb_reg)
    
    # get the R^2
    summary(verb_reg)$r.squared
    
    # which regression "fits the data better"?
    summary(verb_reg)$r.squared > summary(math_reg)$r.squared
...
system("decktape chapter3.html chapter3.pdf --chrome-arg=--disable-web-security")
```