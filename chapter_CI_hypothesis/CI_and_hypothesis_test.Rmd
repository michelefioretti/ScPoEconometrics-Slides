...
We'll focus on simulation to give you the intuition and come back to the maths approach in the next weeks.
...
set.seed(1234)
my_sample = bowl %>%
  mutate(color = as.factor(ifelse(color == "green","green","non-green"))) %>% # Let's focus on either "green" or "non-green" rather than 3 different colours (see `unique(bowl$color)` or `levels(bowl$color)`)
  rep_sample_n(size = 50) %>% ungroup() %>%
  select(pasta_ID, color)
```
...
bootstrap_distrib <- my_sample %>%
  # resample 50 draws with replacement a 1000 times
  rep_sample_n(size = 50, reps = 1000, replace = TRUE)  %>% 
  # compute the average number of 
  # green colors in each sample
  group_by(replicate) %>% 
  summarize(stat = mean(color == "green", na.rm = TRUE)) 
...
```{r, eval = FALSE}
# Using the same approach as last week
set.seed(1234)
bootstrap_distrib <- my_sample %>%
  # resample 50 draws with replacement a 1000 times
  rep_sample_n(size = 50, reps = 1000, replace = TRUE)  %>% 
...
# Remember to `set.seed()` to the same number to compare 
# the two approaches (e.g., `set.seed(1234)`)
...
boot_distrib_plot = bootstrap_distrib %>%
  ggplot(aes(x = stat)) +
  ylim(0, max(table(bootstrap_distrib$stat))+10) + 
  geom_histogram(boundary = 0.39, binwidth = 0.02, col = "white", fill = "forestgreen") +
...
boot_distrib_plot +
  ...
  annotate("text", x = 0.345, y = max(table(bootstrap_distrib$stat)), label = paste0("bootstrap distribution mean at ",round(mean(bootstrap_distrib$stat),2)), size = 4)
```
...
boot_distrib_plot +
  ...
  annotate("text", x = p+0.08, y = 125, label = paste0("population proportion = ", round(p,2)), size = 4)
```
...
***Impact of sample size:*** the greater the sample size, the thinner the confidence intervals.
...
* 48 (male) supervisors were given *identical* candidate CVs, differing only with respect to the first name, which was male or female.
  ...
  * Many similar experiments have been conducted with other groups. Arabic, Black, Jewish names or other groups that can be similarly identified. [[1]](https://catalogue-bibliotheque.sciencespo.fr/discovery/fulldisplay?docid=cdi_proquest_journals_233023724&context=PC&vid=33USPC_SPO:SPO&lang=fr&search_scope=MyInst_and_CI&adaptor=Primo%20Central&tab=Everything&query=any,contains,Are%20Emily%20and%20Greg%20More%20Employable%20Than%20Lakisha%20and%20Jamal%3F%20A%20Field%20Experiment%20on%20Labor%20Market%20Discrimination&offset=0),
...
promotions %>% 
  ...
  tally() %>% # handy way to count entries in each gender-decision group
...
visualize(null_distribution, bins = 10, fill = "forestgreen") + 
...
The red-shaded area correponds to the p-value!
...
* The Null plays no role:

  * Type I error arises when *you have enough evidence* for the alternative

  * Type II error arises when *you do not have enough evidence* for the alternative

  
* [Type I and Type II error and COVID-19 antibody testing](https://www.youtube.com/watch?v=Roke98aXAT0): False positives and false negatives
...
* ***Question***: Is the estimated effect statistically significantly different from some other value $z$?
...
# Appendix: code to generate the null distribution using the `infer` package
...
system("decktape CI_and_hypothesis_test.html CI_and_hypothesis_test.pdf --chrome-arg=--disable-web-security")
```