...
```{r,eval=FALSE,echo=FALSE}
  # Question 1
  count_vector <- c(14, 14, 10, 10, 6, 10, 8, 9, 11, 8, 7, 9, 9, 14, 11, 10, 7, 13)
  pasta <- data.frame(count = count_vector, prop_green = round(count_vector/20,2) )
  
  # Question 2
  ghist_data <- ggplot(pasta,
                  aes(x = count_vector)) 
  ...
  ghist2 <- ghist_data +
    geom_histogram(boundary = 0.325, # boundary between two bins
                   binwidth = 0.05 # how large is the binsize
                   )
  ghist2 + theme_bw()
  
  ghist3 <- ghist_data +
    geom_histogram(boundary = 0.325, # boundary between two bins
                   binwidth = 5 # how many bins do you see now?
                   )
  ghist3 + theme_bw()
  
  # Note: see lecture 2 on how to prittify this graph
  
  # Question 3
  # The plot shows the frequency of each of the outcome from randomly sampling pasta 18 times with replacement. 
  # E.g., we never draw 12 green pastas, but we did draw 14 green pastas (3 times).
...
.pull-left[
* We counted the exact number of green, red and yellow pasta in the bowl `r emo::ji("monocle")`
...
library(moderndive)
set.seed(123) # we will all have the same draw!
...
.pull-right[
* Column `replicate` tells us the ID of the sample. Here: `1` since we create only one sample (see option `reps`).
```{r, echo = TRUE}
# display the sample's first 6 rows
virtual_shovel_4 <- bowl %>% 
  rep_sample_n(size = 50, reps = 4)

unique(virtual_shovel_4$replicate)
```
...
1. Obtain 1000 samples of size 50 using the `rep_sample_n()` function from the `moderndive` package. Note: use `set.seed(1234)` to allow replication (`?set.seed()`)
...
```{r,eval=FALSE,echo=FALSE}
  # Question 1
  # Try if you have spare time.

  # Question 2
  pasta <-read.csv("https://www.dropbox.com/s/qpjsk0rfgc0gx80/pasta.csv?dl=1") # remember the "..."
  #  - or - 
  # pasta <- bowl
  ...
  n_size = 50
  n_rep = 1000
  set.seed(1234) # you can put any number `?set.seed`
  virtual_samples_1000 <- pasta %>%
    rep_sample_n(size = n_size, reps = n_rep)
  
  # Question 4
  virtual_prop_green_1000 <- virtual_samples_1000 %>% 
  group_by(replicate) %>% # calculate stat by sample
  summarize(
    prop_green = mean(color == "green", na.rm = TRUE)) 
  virtual_prop_green_1000
  
  # -- check that it is the same as --
  virtual_prop_green_1000_old_way <- virtual_samples_1000 %>% 
  group_by(replicate) %>% # calculate stat by sample
  summarize(
    num_green = sum(color == "green"),
    sample_n = n()) %>% 
  ...
  unique(virtual_prop_green_1000$prop_green == virtual_prop_green_1000_old_way$prop_green)
  
  # Question 5
  ggplot(virtual_prop_green_1000, aes(x = prop_green)) +
  geom_histogram(binwidth = 0.02, 
                 boundary = 0.51,
                 color = "white",
                 fill = "red") +
  scale_y_continuous(breaks = seq(0, 120, by = 20)) + # This option says that we want the y axis to go from 0 to 120, with breaks every 20. adjust this value if you need to do so
  labs(x = "Proportion of 50 pasta that were green",
       y = "Frequency",
       title = paste0("Distribution of ", n_rep, " samples of size ", n_size) ) +
  theme_bw(base_size = 20)
  
  # Question 6
  # 0.5 is the most frequent occurrence, the histogram looks like a `normal density`.
  # The plot was not centered around a number when we took only 33 samples. 
  # Would the plot change if we were to take 10000 random samples instead??
  
  # Question 7
  # let's count the number of observations such as `prop_green` < 20%
  nrow(virtual_prop_green_1000[virtual_prop_green_1000$prop_green < 0.2,])
  # [1] 0
  # what do you conclude?
  # Instead, how likely is it that  we sample 50 pasta of which less than 50% are green?
  nrow(virtual_prop_green_1000[virtual_prop_green_1000$prop_green < 0.5,])
  #[1] 458
  # There are 458 such occurrences our of 1000 draws or
  nrow(virtual_prop_green_1000[virtual_prop_green_1000$prop_green < 0.5,])/nrow(virtual_prop_green_1000) * 100
  # [1] 45.8
  # occurrences
  ...
  labs(
    ...
    facet_wrap(~sample_n, scales = "free_y", labeller = as_labeller(
        c(`25` = "1000 samples of size 25",
          ...
          `100` = "1000 samples of size 100"))) +
    theme_bw(base_size = 14) +
  geom_vline(xintercept = p, col = "red", size = 0.75) +
...
system("decktape chapter6.html chapter6.pdf --chrome-arg=--disable-web-security")
```