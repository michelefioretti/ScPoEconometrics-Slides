...
## Today: ***Difference-in-differences***
...
* In this lecture, we will cover a popular and rigorous program evaluation method: __difference-in-differences__.
...
* Imagine you are interested in assessing the __causal__ impact of increasing the minimum wage.
...
?njmin
    ...
    # chain = fast food chain
    # region = pa1 is northeast suburbs of Phila, pa2 is Easton etc, centralj is central NJ, northj is northern NJ, southj is south NJ
    # observation = Date of first (February 1992) and second (November 1992) observation.
    # ncalls = Number of call-backs. Is 0 if contacted on first call.
...
ck1994 %>% 
  ...
  summarise(count = n())
  # 410 stores == "Number of Interviewed Stores"

# Question 3
ck1994$empfte <- ck1994$empft + 0.5*ck1994$emppt + ck1994$nmgrs # see page 475 of the paper

# Question 4
ck1994 %>% 
  group_by(state, observation) %>% 
  summarise(avg_empfte = mean(empfte, na.rm = T),
            per_empfte = mean(empft / empfte, na.rm = T)*100,
            avg_wage_st = mean(wage_st, na.rm = T))

# Question 5
ck1994 %>% 
  group_by(state, observation) %>% 
  summarise(n = n(),
            avg_empfte = mean(empfte, na.rm = T),
            se_avg_empfte = sd(empfte, na.rm = T)/sqrt(n),
            per_empfte = mean(empft / empfte, na.rm = T)*100,
            se_per_empfte = sd(empft / empfte*100, na.rm = T)/sqrt(n),
            avg_wage_st = mean(wage_st, na.rm = T),
            se_avg_wage_st = sd(wage_st, na.rm = T)/sqrt(n)) %>%
  group_by(state) %>%
  summarise(diff_avg_empfte = diff(avg_empfte),
            diff_per_empfte = diff(per_empfte),
            diff_avg_wage_st = diff(avg_wage_st),
            t_test_avg_empfte = diff(-avg_empfte)/sqrt(sum(se_avg_empfte^2)),
            t_test_per_empfte = diff(-per_empfte)/sqrt(sum(se_per_empfte^2)),
            t_test_avg_wage_st = diff(-avg_wage_st)/sqrt(sum(se_avg_wage_st^2)))

# Note: consider whether to include standard errors of t-test
# formula for t.test <- http://www.sthda.com/english/wiki/unpaired-two-samples-t-test-in-r
...
Difference-in-differences causal estimate: $0.59 - (-2.17) = 2.76$
...
Yes the essence of difference-in-differences is _that_ simple! `r emo::ji("grinning")`
...
# Question 1
ck1994$treat = FALSE
ck1994$treat[ck1994$state == "New Jersey"] = TRUE

# Question 2
ck1994$post = FALSE
ck1994$post[ck1994$observation == "November 1992"] = TRUE

# Question 3
summary(lm(empfte ~ treat*post, data = ck1994))
...
system("decktape chapter_did.html chapter_did.pdf --chrome-arg=--disable-web-security")
```