...
# Recap from last week NOTE: UPDATE THIS SLIDE!!
...
# Question 2
  summary(reg <- lm(avgverb ~ classize + disadvantaged, data = grades))
  
# Question 3
  reg$coefficients
  
# Question 4
  summary(reg_full <- lm(avgverb ~ classize + disadvantaged + school_enrollment  + female + religious, data = grades))
```
...
1. In the same dataset, add a dummy variable `star_df$small` equal to `TRUE` if students are in a small class and `FALSE` if they are in a regular class. In the same way, add a dummy variable `star_df$regular` equal to `TRUE` if students are NOT in a small class and to `FALSE` if they are in a small class. Clearly these two variables are perfectly collinear.
...
# Quesiton 2
  star_df$small = FALSE 
  star_df$small[star_df$star == "small"] = TRUE
  
  star_df$regular = FALSE 
  star_df$regular[star_df$star != "small"] = TRUE
  
# Question 4
  lm(small ~ math, data = star_df)
  
# Question 5
  lm(regular ~ math, data = star_df)
  
# Question 6
  summary(lm(small ~ math + regular, data = star_df))
```
...
```{r echo = FALSE}
reg_full <- lm(avgmath ~ classize + disadvantaged + school_enrollment + female + religious, grades)
...
```{r, eval=FALSE,echo=FALSE}
# Question 1
  grades$avgmath_stand <- standardize(grades$avgmath)

# Question 2
  reg_full2 <- lm(avgmath_stand ~ classize + disadvantaged + school_enrollment + female + religious, grades)
  reg_full2$coefficients
  
# Question 3
  # hard to tell... we need to standardize the variables
  
# Question 4
  grades$classize_stand <- standardize(grades$classize)
  grades$disadvantaged_stand <- standardize(grades$disadvantaged)
  grades$school_enrollment_stand <- standardize(grades$school_enrollment)
  grades$female_stand <- standardize(grades$female)
  grades$religious_stand <- standardize(grades$religious)
  
  # No because it is a dummy variable. 
  reg_full3 <- lm(avgmath_stand ~ classize_stand + disadvantaged_stand + school_enrollment_stand + female_stand + religious, grades)
  reg_full3$coefficients
  
  reg_full4 <- lm(avgmath_stand ~ classize_stand + disadvantaged_stand + school_enrollment_stand + female_stand + religious_stand, grades)
  reg_full4$coefficients
...
# Question 3
  # it matters for instance if women (or men) are more likely to work part-time. 
  # If we find that women (or men) earn less, but we do not observe part-time/full-time employment, and part-time 
  # workers have smaller wages than full-time workers, then we may mistakenly associate the wage gap between men and women
  # to gender differences rather than to the decision to work part-time or full-time.
  
# Question 4
  df$log_wage <- log(df$wage)
  
# Question 5
  reg1 <- lm(log_wage ~ gender + education, data = df)
  
# Question 6
  reg2 <- lm(log_wage ~ gender*education, data = df)
  
# Question 7
  reg3 <- lm(log_wage ~ gender*education + experience + age +ethnicity + region+ occupation + sector+ union + married, data = df)
...
system("decktape chapter4.html chapter4.pdf --chrome-arg=--disable-web-security")
```