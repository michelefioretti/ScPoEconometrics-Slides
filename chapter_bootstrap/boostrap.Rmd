...
p1 = ggplot(pennies_resample, aes(x = year)) +
  ...
  labs(title = "Resample of 50 pennies")+theme_bw()
p2 = ggplot(pennies_sample, aes(x = year)) +
  geom_histogram(binwidth = 10, color = "white",boundary = 1990) +
  labs(title = "Original sample of 50 pennies")+theme_bw()
...
# e.g., rep_sample_n(tibble(pennies$year), size = nrow(pennies), replace = TRUE) - Don't show
## NOTE: update google sheet
```
...
ggplot(resampled_means, aes(x = mean_year)) +
  ...
  labs(x = "Sampled mean year") + theme_bw()
```
]
...
ggplot(virtual_resampled_means, aes(x = mean_year)) +
  ...
  geom_vline(xintercept = percentile_ci[[1, 2]], size = 1)  + theme_bw()
...
ggplot(virtual_resampled_means, aes(x = mean_year)) +
  ...
  geom_vline(xintercept = standard_error_ci[[1, 2]], linetype = "dashed", size = 1)  + theme_bw()
```
]
...
```{r,fig.height=5}
visualize(bootstrap_distribution)  + theme_bw()
```
]
...
sample_1_bootstrap %>% 
  ...
  geom_vline(xintercept = 0.375, linetype = "dashed") + theme_bw()
```
]
...
ggplot(percentile_cis) +
  ...
  theme(panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank())  + theme_bw()
```
]
...
system("decktape bootstrap.html bootstrap.pdf --chrome-arg=--disable-web-security")
```