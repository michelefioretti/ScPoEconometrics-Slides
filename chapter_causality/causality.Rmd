...
# Question 3
# The same student is only in one class at any point in time. Outcome of "RESHAPE" # NOTE: check it with Florian!
...
star_df %>%
  group_by(star,grade) %>%
  mutate(share_female = mean(gender == "female") * 100,
         share_afam = mean(ethnicity == "afam") * 100,
         share_lunch = mean(lunch == "free") * 100) %>%
  summarise(mean_share_female = mean(share_female, na.rm = T),
            mean_share_afam = mean(share_afam, na.rm = T),
            mean_share_lunch = mean(share_lunch, na.rm = T))
