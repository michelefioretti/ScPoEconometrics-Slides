...
* In the next two lectures you will learn the __basics__ of summarizing, visualising and tidying data
...
```{r,eval = FALSE}
    var(x)
    range(x)   # range: max value - min value
    ```
]
---


# Example: the Weight of Women and Men<sup>1</sup>

.pull-left[
* Men weight 65 kg and women 55 kg on average. The variance is 25.
    ```{r, echo = TRUE}
    # Generate a random dataset
    set.seed(1234) # `set.seed` allows replicating random data
    df <- data.frame(
        sex=factor(rep(c("F", "M"), each=200)),
        weight=round(c(rnorm(200, mean=55, sd=5), # ?rnorm | Note: sd = sqrt(variance)
...
* Plot separated densities
    ...
    ggplot(df, aes(x=weight, color=sex)) + geom_density() + theme_bw() # White background
    ```
]
...
ggplot(df, aes(x=weight)) + 
          geom_density() +
          # Additional option 1: mean line
          geom_vline(aes(xintercept=mean(weight)),
                   color="blue", linetype="dashed", size=1) +
          ggtitle("Overall density") +
          theme_bw() # Additional option 2: white background
```
...
```{r,echo = FALSE,fig.height=3,message = FALSE,warning = FALSE}
    ...
    ggplot(df, aes(x=weight, color=sex)) +
      geom_density()+
      # Additional option 1: mean
      geom_vline(data=mu, aes(xintercept=grp.mean, color=sex),
                 linetype="dashed") + 
      ggtitle("Density by sex") +
      theme_bw() + # Additional option 2: white background
      theme(legend.position="bottom") 
    ```

.footnote[
[1]: This example is taken from [sthda.com](http://www.sthda.com/english/wiki/ggplot2-density-plot-quick-start-guide-r-software-and-data-visualization).  
]
...
* `r emo::ji("warning")` To obtain `table`s with `NA`s, use the `useNA = "always"` or `useNA = "ifany"`
---
...
***Your turn:*** How does the same plot look like for `fertility`? For `infant_mortality`?
]
...
plot(fertility ~ infant_mortality,
    ...
    ylab = "Fertility rate",
    xlim = c(0,200), # range of x axis
    ylim = c(0,8), # range of y axis
...
ggplot(gapminder_2015,
  ...
  geom_histogram(binwidth = 5,
                 boundary = 0, # try with 1: what does it do?,
                 colour = "white",
                 fill = "#d90502") +
  labs(x = "Life Expectancy",
       y = "Frequency",
       title = "Histogram of life expectancy in 2015") +
  theme_bw(base_size = 16) # white background + base font size adjusted to 16 pts
```
]
...
ggplot(gapminder_2015,
       aes(x = life_expectancy)) +
  geom_histogram(binwidth = 5, boundary = 0, colour = "white", fill = "#d90502") +
...
* [This](https://michelefioretti.github.io/ScPoEconometrics/sum.html#summarize-two) is the relevant section in the book about Covariance. 
...
`r emo::ji("rotating_light")` This is a `tibble` (more informative than a  `data.frame`)

What variables does this dataset contain? 

--

1. `?polls_us_election_2016`
1. [FiveThirtyEight’s Pollster Ratings website](https://projects.fivethirtyeight.com/pollster-ratings/)
...
# Question 1
  df1 <- filter(polls_us_election_2016, rawpoll_clinton <= rawpoll_trump)
  dim(df1)

# Question 2
  df2 <- filter(polls_us_election_2016, is.na(grade) == TRUE)
  dim(df2)
  
# Question 3
  df3 <- filter(polls_us_election_2016, pollster %in% c("American Strategies", "GfK Group", "Merrill Poll"), samplesize > 1000)

# Question 4  
  polls_us_election_2016_mutated4 <- mutate(polls_us_election_2016, rawpoll_clinton_trump = rawpoll_clinton + rawpoll_trump)
  dim(polls_us_election_2016)
  dim(polls_us_election_2016_mutated4 )   # we have a column more now!
  df4 <- filter(polls_us_election_2016_mutated4, is.na(rawpoll_johnson) == TRUE,  rawpoll_clinton_trump > 0.95 )

# Question 5
  polls_us_election_2016_mutated5 <- mutate(polls_us_election_2016, rawpoll_trump_clinton_diff = rawpoll_trump - rawpoll_clinton)
  dim(polls_us_election_2016)
  dim(polls_us_election_2016_mutated5 )   # we have a column more now!
  df5 <- filter(polls_us_election_2016_mutated5, is.na(rawpoll_johnson) == TRUE,  rawpoll_trump_clinton_diff > 5 )
...
system("decktape chapter2.html chapter2.pdf --chrome-arg=--disable-web-security")
```