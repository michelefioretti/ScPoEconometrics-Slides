...
1. Generate the bootstrap distribution of $b_\textrm{small}$ based on 1000 samples drawn from `star_df`. *Hint*: use the appropriate functions and arguments from either the `infer` or `moderndive` package so use the help pages.
...
set.seed(123)
# - or, alternatively, with some more code (but potentially cleaner) -
boot_alt <- star_df %>%
  # resample with replacement a number of times
  rep_sample_n(size = nrow(star_df), reps = repetitions, replace = TRUE) %>%
  # group the resulting `data.frame` by the ID of each repetition (we have 1000 samples)
  group_by(replicate) %>%
  # we propose 2 alternative ways to compute the standard error: (i) as the standard deviation of the `small beta` coefficient in each regression, and (ii) as the average of the standard error of the `small beta` coefficient in each regression. 
  # To do that we run a regression in each sample using `summarise`
  summarise(beta = summary(lm(math ~ small))$coefficients[2,1], 
            se = summary(lm(math ~ small))$coefficients[2,2]) %>%
  ungroup() %>%
  summarise(st_error = round(sd(beta),3),
         st_error_same_alternative_way = round(mean(se),3)) # These two numbers should be reeeeally close to each others, and to `se_simul`. Try changing the variable `repetitions`: what does it happen to the standard error if you increase the number of repetitions? If you decrease it instead?
...
.pull-left[
```{r}
observed_stat = lm(math ~ small, star_df)$coefficients[2]/sd(bootstrap_distrib$stat)
...
plot_null_distrib = null_distribution %>%
  ...
  theme_bw(base_size = 14) +
  xlim(min(null_distribution$test_stat)-0.5, max(null_distribution$test_stat,observed_stat)+0.5)
plot_null_distrib
```
...
Very unlikely to obtain $b_\textrm{small}$ = `r reg_summary_star_df[2]` when $H_0$ is true.
...
* Theory-based approach uses one fundamental result: the sampling distribution of the sample statistic $\frac{b - \beta}{\hat{\textrm{SE}(b)}}$ *converges* to a ***standard normal distribution*** as the sample size gets larger and larger.
  ...
  * It is also obtained through a theoretical formula (which you can find in the [book](https://michelefioretti.github.io/ScPoEconometrics/std-errors.html#se-theory)!) but we'll leave it aside.
...
```{r}
tidy(lm(math ~ small, star_df),
     conf.int = TRUE, conf.level = 0.95) %>% # To display confidence intervals
...
# Question 1
quantile(bootstrap_distrib$stat, probs = c(0.025, 0.975))
...
tidy(lm(math ~ small, star_df),
  ...
  filter(term == "smallTRUE") %>%
  select(term, conf.low, conf.high) %>%
  mutate(diff.conf.low = conf.low - quantile(bootstrap_distrib$stat, 0.025),
         diff.conf.high = conf.high - quantile(bootstrap_distrib$stat, 0.975)) %>%
  select(diff.conf.low, diff.conf.high) # Quite small! We good!
...
* Before delving into these assumptions, let's see the small but important modifications we apply to our model (back to [*lecture 4: Simple Linear Regression*](https://raw.githack.com/michelefioretti/ScPoEconometrics-Slides/master/chapter3/chapter3.html#1)):
...
library(AER)
data(CPS1985)
cps <- CPS1985 # rename it for simplicity of typing
head(df)
...
summary(reg2)
# a year more of education increases `log_wages` for women by 0.03 more than for `men`.
# The coefficient is not significant at 5% level, but it is significant at 10% level.
...
```{r,eval=FALSE,echo=FALSE}
# Question 1
geduc_wage <- ggplot(cps, aes(x=education, y=log_wage))+
  geom_point(size = 2, alpha = 0.5, position = "jitter", color = "#d90502")+ # jitter + Sciences Po Red color!
  xlim(0,20) +
    ylim(-0.1, 4) +
    labs(
      x = "Education",
      y = "Log(Wage)",
      title = "Relation between log(wage) and education") +
    theme_bw(base_size = 20)

geduc_wage

# we see some bunching at 12ish, but an overall increasing relationship. Some outliers for log(wage) = 0 and close to 4.

# Question 2
geduc_wage + geom_smooth() #`geom_smooth()` computes a smooth local regression. A bit fancy, but basically it just tells you how `y` (i.e., log_wage) changes in a small neighborhood of `x` (i.e., education).

# Question 3
cps_boot <- cps %>%
  rep_sample_n(size = nrow(cps), reps = 1, replace = TRUE) # bootstrap --> " replace = TRUE "

reg_bootstrap <- lm(log_wage ~ gender * education , cps_boot)
summary(reg_bootstrap)

intercept_men_bootstrap <- summary(reg_bootstrap)$coefficients[1,1]
slope_men_bootstrap <- summary(reg_bootstrap)$coefficients[3,1]

intercept_women_bootstrap <-intercept_men_bootstrap  + summary(reg_bootstrap)$coefficients[2,1]
slope_women_bootstrap <- slope_men_bootstrap + summary(reg_bootstrap)$coefficients[4,1]
...
system("decktape reg_inference.html reg_inference.pdf --chrome-arg=--disable-web-security")
```