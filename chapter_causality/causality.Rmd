...
# Question 1
avg_death_rate <- (15*11+35*13+50*16)/40
avg_death_rate

# Question 2
avg_death_rate2 <- (15*29+35*9+50*2)/40
avg_death_rate2
...
```{r,eval=FALSE,echo=FALSE}
## NOTE: review this exercise
# Question 1 
data = read.csv("https://www.dropbox.com/s/a4u67nmu9gnl94y/toy_data.csv?dl=1")
head(data)

# Question 2
data$Y_i = data$D_i * data$Y1_i + (1-data$D_i) * data$Y0_i
data$delta_i = data$Y1_i - data$Y0_i

# Question 3
ATE = mean(data$Y1_i) - mean(data$Y0_i)
round(ATE,5) == round(mean(data$delta_i),5) # ATE == delta
SDO = mean(data$Y_i[data$D_i == 1]) - mean(data$Y_i[data$D_i == 0])
bias = SDO - ATE

# Question 4
data$Y_i_random = data$D_i_random * data$Y1_i + (1-data$D_i_random) * data$Y0_i
SDO_random = mean(data$Y_i_random[data$D_i_random == 1]) - mean(data$Y_i_random[data$D_i_random == 0])
bias_random = SDO_random - ATE

# Question 5
selection_bias = mean(data$Y0_i[data$D_i == 1]) - mean(data$Y0_i[data$D_i == 0])
ATT = mean(data$Y1_i[data$D_i == 1]) - mean(data$Y0_i[data$D_i == 1])
ATU = mean(data$Y1_i[data$D_i == 0]) - mean(data$Y0_i[data$D_i == 0])
pi <- mean(data$D_i) # share in the treatment group
heterogeneout_treatment_effect_bias = (1-pi) * (ATT - ATU)

SDO - (ATE + selection_bias + heterogeneout_treatment_effect_bias)
```
---
...
* In this case, the treatment status is **independent** of the potential outcomes.
...
# Question 2
head(star_df)
nrow(star_df) # NOTE: why so many??? 4 years of experiment?

# Question 3
# Each row correspond to a student in a specific grade. The unit of observation is student by grade. 
# (i)   the class assignment is `star`
# (ii)  the student's class grade is `grade`
# (iii) the outcomes of interest are `read` and `math`

# Question 4
nrow(star_df) # number of observations
ncol(star_df) # number of variables
# There are many observations because the dataset is in its `wide` format: it includes also observations
# for students whose records are missing for some grades because the STAR experiment started after they
# completed those grades.
...
star_df %>%
  group_by(star,grade) %>%
  summarise(share_female = mean(gender == "female", na.rm = T),
            share_afam = mean(ethnicity == "afam", na.rm = T),
            share_lunch = mean(lunch == "free", na.rm = T))
...
1. Create a dummy variable `treatment` equal to `TRUE` if student is in treatment group (i.e. small class size) and `FALSE` if in control group (i.e. regular class size).
...
# Question 2
avg_math_small = mean(star_sr$math[star_sr$star == "small"])
avg_math_small
avg_math_regular = mean(star_sr$math[star_sr$star == "regular"])
avg_math_regular
avg_diff = avg_math_small - avg_math_regular
avg_diff

# Question 3
star_sr$treatment = FALSE
star_sr$treatment[star_sr$star == "small"] = TRUE

# Question 4
reg <- lm(math ~ treatment , data = star_sr)

# Question 5
# Belonging to a small class improves a student's math score by 13.16 points on average, ceteris paribus.
```
...
system("decktape causality.html causality.pdf --chrome-arg=--disable-web-security")
```